...
function removeFirstOccurrences(str, value) {
  const left = str.indexOf(value);
  const right = left + value.length;
  return left < 0 ? str : str.slice(0, left) + str.slice(right);
}
...
function removeLastOccurrences(str, value) {
  const left = str.lastIndexOf(value);
  const right = left + value.length;
  return left < 0 ? str : str.slice(0, left) + str.slice(right);
}
...
function sumOfCodes(str) {
  ...
  if (isString(str)) {
    ...
    result = str.split('').reduce(reducer, 0);
  }
...
function countVowels(str) {
  const matchingInstances = str.match(/[aeiouy]/gi);
...
function isPalindrome(str) {
  const re = /[^a-z0-9]/g;
  const text = str.toLowerCase().replace(re, '');
...
function extractEmails(str) {
  return str.split(';').filter((curr) => curr.includes('@'));
}
